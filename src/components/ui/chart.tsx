 ...
 * - Chart configurations MUST be static and defined in code
 * - NEVER pass user-controlled data to chart configs without validation
 * - If user input is needed, validate colors against safe patterns (hex, rgb, hsl)
 * - Chart IDs are component-generated and should not be user-controlled
 * 
 * Current implementation is safe because:
 * - Colors come from predefined theme objects
 * - Chart IDs are generated by React.useId()
 * - No user input flows to this component
 */

/**
 * Sanitizes CSS color values to prevent injection attacks
 * Only allows safe color formats: hex, rgb, rgba, hsl, hsla, and CSS color names
 */
const sanitizeCSSColor = (color: string): string | null => {
  ...
  // Validate against safe color patterns
  const safePatterns = [
    ...
    /^hsl\(\s*\d{1,3}\s*,\s*\d{1,3}%\s*,\s*\d{1,3}%\s*\)$/, // hsl(0, 0%, 0%)
    /^hsla\(\s*\d{1,3}\s*,\s*\d{1,3}%\s*,\s*\d{1,3}%\s*,\s*[\d.]+\s*\)$/, // hsla(0, 0%, 0%, 0.5)
    /^[a-z]+$/, // CSS color names (red, blue, etc.)
  ];
...
const sanitizeCSSKey = (key: string): string | null => {
  ...
  // Only allow safe CSS variable names
  if (!/^[a-zA-Z0-9_-]+$/.test(trimmed)) {
    console.warn(`[Security] Invalid CSS key rejected: ${key}`);
    return null;
  }
